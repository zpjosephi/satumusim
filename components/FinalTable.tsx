...
export default function FinalTable() {
  ...
  return (
    <section id="table" ref={root} className="edge py-32 md:py-44">
      <div className="grid items-end gap-6 md:grid-cols-[1fr_auto]">
        <h2 className="track-head font-display text-3xl font-semibold md:text-5xl">
          How it ended
        </h2>
        <p className="max-w-[44ch] text-muted md:text-right">
          Two clubs finished on 97 points or more. Nobody else reached 73.
        ...
        {FINAL_TOP6.map((row, i) => (
          <div key={row.team} role="listitem" className="group">
            {i === 2 && (
              <p
                aria-hidden="true"
                className="track-label border-t border-dashed border-[var(--line)] py-3 font-mono text-xs text-muted"
              >
                25 POINTS BACK TO THIRD
              </p>
            )}
            <div className="grid grid-cols-[8rem_1fr] items-center gap-4 py-1 sm:grid-cols-[13rem_1fr]">
              <span
                data-row-label
              ...
              <div className="relative flex items-center gap-3">
                <div
                  data-bar
                  className="h-9 rounded-r md:h-11"
                  style={{
